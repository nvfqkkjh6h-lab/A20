...
function saveData() {
  try {
    localStorage.setItem('my_text_app_data_v4', JSON.stringify(appData));
  } catch(e) {
...
// データ読み込み
function loadData() {
  const rawData = localStorage.getItem('my_text_app_data_v4') || localStorage.getItem('my_text_app_data_v3');
...
function exportDataAsJSON() {
  ...
  downloadAnchor.setAttribute("download", `my_text_backup_${new Date().toISOString().split('T')[0]}.json`);
...
function importDataFromJSON(e) {
  ...
  reader.onload = function(event) {
    try {
      ...
      if (imported && Array.isArray(imported.questions)) {
        appData = imported;
        saveData();
        applyTheme(appData.theme || 'theme-purple');
        updateSubjectFilterOptions();
